...
class BankAccountFilter(BaseFilter):
    ...
    def _fetch_bank_accounts(self):
        ...
        bank_accounts.update(
            {str(account.id): account.account_name for account in self.model.objects.all().order_by("account_name")}
        )
        return bank_accounts

    def _update_selected(self):
        """Callback function to update session state when the pills selection changes."""
        selected = st.session_state.get(f"bank_account_filter")
        st.session_state[f"selected_bank_accounts"] = selected

    def _select_all(self):
        """Callback function to select all bank accounts."""
        options = list(self.bank_accounts.values())
        st.session_state[f"selected_bank_accounts"] = options

    def _deselect_all(self):
        """Callback function to deselect all bank accounts."""
        st.session_state[f"selected_bank_accounts"] = []
    ...
    def place_widget(self, sidebar=False):
        ...
        if f"selected_bank_accounts" not in st.session_state:
            st.session_state[f"selected_bank_accounts"] = options
        ...
        with location.expander(f"{self.label}", expanded=True):
            selected = st.pills(
                ...
                default=st.session_state[f"selected_bank_accounts"],
                key=f"bank_account_filter",
            ...
            with col1:
                if st.button("Select All", key="select_all_bank_accounts", on_click=self._select_all):
                    pass  # The actual change will happen in the callback
            with col2:
                if st.button("Deselect All", key="deselect_all_bank_accounts", on_click=self._deselect_all):
                    pass  # The actual change will happen in the callback

            # Map selected options back to bank account IDs
            selected_ids = [key for key, value in self.bank_accounts.items() if value in selected]
